...
if(!isset($_GET["id"]))
{
	...
	$get_id_suhu=$connect -> query("SELECT * FROM probabilitas_kriteria_jenis_tanah WHERE id = ".$id_tb_suhu);
...
<body>
	...
	<h3>Edit Data Jenis Tanah</h3>
	<form action="edit-process-jns-tnh.php" method="post">
		<input type="hidden" name="id-suhu" value="<?= $_GET["id"] ?>">
		<table>
			<tr>
				<td>Suhu</td>
				<td>
					<input type="text" name="suhu__" value="<?= $get_id_suhu["jenis_tanah"] ?>">
				</td>
			</tr>
			<tr>
